...
- **MoTeC LD** – always written when exporting to CSV (single-file mode). Channel names come from **`[export.motec] profile`** and `motec_profiles/<profile>.toml` (built-in: `rbr`, `rally`).
